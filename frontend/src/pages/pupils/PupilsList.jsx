...
const PupilsList = () => {
  ...
  // ── Print Pupils Register (downloads PDF) ──
  const handlePrint = async () => {
    toast('Generating register…', { icon: '🖨️', duration: 1500 });
    ...
    doc.text(`PUPILS REGISTER — ${filterClassName}`, pw / 2, y, { align: 'center' });
    y += 6;
    ...
    const rows = allPupils.map((p, i) => {
      ...
      return [i + 1, `${p.first_name} ${p.last_name}`, p.class_name || '—', p.gender || '—', p.admission_number || '—', dob];
    });
  ...
  const handleDownloadPDF = async () => {
    ...
    doc.setFont(undefined, 'normal');
    doc.setTextColor(80, 80, 80);
    doc.text(`PUPILS REGISTER — ${filterClassName}`, doc.internal.pageSize.getWidth() / 2, y, { align: 'center' });
    y += 6;
  ...
  // ── Print Blank Admission Form (generates a PDF) ──
  const handlePrintAdmissionForm = async () => {
    ...
    // ─── Photo box (top-right, blank placeholder) ───
    const photoBoxW = 26;
    const photoBoxH = 32;
    ...
    // ─── Helper: section title (clean gray) ───
                  ...
                  <td className="p-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500 sm:hidden">{pupil.class_name || '—'}</p>
                      </div>
                    </div>
                  </td>
                  <td className="p-3 text-gray-600 hidden sm:table-cell">{pupil.class_name || '—'}</td>
                  <td className="p-3 text-gray-600 hidden md:table-cell">{pupil.gender || '—'}</td>
                  <td className="p-3 text-gray-600 hidden lg:table-cell">{pupil.admission_number || '—'}</td>
